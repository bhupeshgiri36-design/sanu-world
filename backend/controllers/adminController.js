...
export const adminController = {
  ...
  getStats: async (req, res) => {
    ...
    let totalMessages = 0;
    rooms.forEach(room => {
      if (room.code === 'DEMO12') return;
      totalOnline += (room.members?.size || 0);
      totalMessages += (room.messages?.length || 0);
    });
    res.json({
      activeRooms: rooms.filter(r => r.code !== 'DEMO12').length,
      totalOnline,
      totalMessages
    });
  },
  getRooms: async (req, res) => {
    ...
    const roomsList = rooms
      // DEMO12 is an internal fixture room (keeps local dev/testing working
      // without a real login) — it shouldn't show up as a "real" room here.
      .filter(room => room.code !== 'DEMO12')
      .map(room => ({
        code: room.code,
        name: room.name,
