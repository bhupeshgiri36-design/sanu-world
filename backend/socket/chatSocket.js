  ...
  io.on('connection', (socket) => {
    ...
    on('join-room', async ({ roomCode, nickname, password, clientId } = {}, callback) => {
      ...
      await withRoomLock(code, async () => {
        ...
        // nickname is still just a friend. DEMO12 keeps the old "first person
        // is host" behavior so the demo room still works without a real login.
        ...
        // in — Sanu has to approve them first. Admin joins and the DEMO12
        // sandbox room skip this entirely.
    ...
    on('respond-join-request', async ({ requestId, approve } = {}, callback) => {
      ...
      await withRoomLock(currentRoom, async () => {
        ...
        // 🔧 FIX: Create the new user object
        const newUser = { 
          id: request.socketId, 
        ...
        // This is what was missing — without this, the socket's currentRoom/currentUser 
        // variables stay null, and any send-message attempt fails with "Not connected to a room"
        // We need to set these on the requesterSocket's data object so they're accessible
        // to all future handlers for that socket.
        requesterSocket.data.currentRoom = currentRoom;
        requesterSocket.data.currentUser = newUser;

        // Join the socket to the room's namespace
        requesterSocket.join(currentRoom);

        // Send the approval response to the requester with full room data
